...
class Interface {
  ...
  getOperationsWithArgsOfType (types) {
    let found = []
    let alreadyFound = new Set()
    for (const [opName, opt] of this.operations.entries()) {
      if (!alreadyFound.has(opName) && opt.hasArgOfTypes(types)) {
  ...
  getOperationsWithArgsOfTypeTrackingSeen (types, seen) {
    let found = []
    for (const [opName, opt] of this.operations.entries()) {
      if (!seen.has(opName) && opt.hasArgOfTypes(types)) {
        found.push(opt)
        seen.add(opName)
        // console.log(this.name, opName,opt.argsNames())
      }
  ...
  getConstructorsWithArgsOfType (types) {
    let found = []
    let seen = new Set()
    for (let i = 0; i < this.construtors.length; ++i) {
      let c = this.construtors[i]
      if (c.hasArgOfTypes(types)) {
        let names = c.argsNames()
        if (!seen.has(names)) {
          seen.add(names)
          found.push(c)

...
class EG {
  constructor () {
    this._overridesBody = []
    this._curStatements = []
    this._typedefInfo = null
  }

  addTypeDefInfo (td) {
    this._typedefInfo = td
  }
  ...
  extendHTML (elem) {
    ...
    if (jsToTag[name]) {
      ...
      let idArg
      if (Array.isArray(constructor.identifiedArg)) {
        idArg = constructor.identifiedArg[0]
      } else {
        idArg = constructor.getIdentifiedArgs()[0]
      }
  ...
  handleRequest (iface) {
    if (iface.hasIdConstructors()) {
      let constructor = iface.constructors[0]
      let args = []
      for (let i = 0; i < constructor.args.length; ++i) {
        let arg = constructor.args[i]
        if (iface.identifiedArgs.size === 0) {

        }
        if (constructor.hasTypeDefArgs &&
          constructor.typeDefArgs.indexOf(arg.name) !== -1) {
          if (arg.typedefInfo.for.has(iface.name)) {
            console.log(`${arg} is typedefed to iface`)
          }
          if (constructor.identifiedArgs.has(arg.name)) {
            if (arg.readonly) {
              console.log(`${arg} is readonly`)
            }
          }
        } else {
          if (constructor.identifiedArgs.has(arg.name)) {
  ...
  argNameToId (arg) {
    if (typeof arg === 'object') return btypes.identifier(arg.name)
    return btypes.identifier(arg)
  }

  getArgNames (hasArgs) {
    if (Array.isArray(hasArgs)) {
      return hasArgs.map(this.argNameToId)
    }
    if (typeof hasArgs.what === 'function') {
      switch (hasArgs.what()) {
        case 'operation':
          return hasArgs.argsNames().map(this.argNameToId)
      }
    }
    console.log(hasArgs)
    throw new Error(`Can not get arg names of ${hasArgs}`)
  }
  ...
  handleDocument (iface) {
    ...
    for (let i = 0; i < operations.length; ++i) {
      ...
      const args = this.getArgNames(operation)
      const arg = args[0]
